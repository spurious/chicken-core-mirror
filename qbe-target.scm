          ...
          (else
            (let ((r (apply (cdr s) (cdr x))))
              (cond ((pair? r)
                    ...
                    ((and (not stmt) (not (symbol? r)))
                     (bomb "target - intrinsic in expression context does not return temporary" x))
                    (else r)))))))
...
(define-intrinsic (C_demand n)
  (let ((t1 (temp))
        (t2 (temp))
        (t3 (temp))
        (t4 (temp))
        (tn (expression n)))
    (emit #t "%" t1 " =l alloc4 1"
          #t "%" t2 " =l sub %" t1 ",$C_stack_limit"
          #t "%" t3 " =l add %" tn ",$C_scratch_usage"
          #t "%" t4 " =w cgtl %" t2 ",%" t3)
    t4))
  
(define-intrinsic (C_calculate_demand n c m)
  `(+ ,n (cond (> ,c ,m) 0 ,m)))
